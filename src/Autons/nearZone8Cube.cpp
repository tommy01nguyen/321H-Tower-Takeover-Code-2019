...
void nearZone8Cube(int side){ //This the op op one
  ...
  //Start Up
  chassisProfile.generatePath({Point{0_in, 0_in, 0_deg}, Point{50_in, 0_in, 0_deg}}, "through first row");
  flipout(); //flipout
  setintakeState(intakeStates::on, 12000);

  //First Row
  chassisProfile.setTarget("to first row");
  if(side == red) chassisProfile.generatePath({Point{0_in, 0_in, 0_deg}, Point{40_in, 20_in, 0_deg}}, "spline to second row");
  if(side == blue) chassisProfile.generatePath({Point{0_in, 0_in, 0_deg}, Point{40_in, -20_in, 0_deg}}, "spline to second row");
  chassisProfile.waitUntilSettled();

  //Second Row
  chassisProfile.setTarget("spline to second row", true);
  chassisProfile.removePath("to first row");
  chassisProfile.generatePath({Point{0_in, 0_in, 0_deg}, Point{40_in, 0_in, 0_deg}}, "through second row");
  chassisProfile.waitUntilSettled();
  chassisProfile.setTarget("through second row");
  chassisProfile.removePath("spline to second row");
  chassisProfile.generatePath({Point{0_in, 0_in, 0_deg}, Point{40_in, 0_in, 0_deg}}, "out of second row");
  chassisProfile.waitUntilSettled();

  //Align with Zone
  chassisProfile.setTarget("out of second row", true);
  chassisProfile.removePath("through second row");
  chassisProfile.generatePath({Point{0_in, 0_in, 0_deg}, Point{40_in, 0_in, 0_deg}}, "align with zone");
